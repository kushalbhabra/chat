import { encodeThreadId, decodeThreadId, channelIdFromThreadId, isDM as isDMThread, } from './utils/thread-utils.js';
import { getAccessToken, validateToken } from './utils/auth-validator.js';
import { activityToMessage, messageToActivity, renderMarkdown, adaptiveCardToAttachment, } from './utils/format-converter.js';
import { TeamsAdapterError, TeamsAdapterErrorCode, handleApiError, isRateLimitError, handleHttpError, } from './utils/error-handler.js';
...
class TeamsAppImpl {
    ...
    async dispatch(event, activity, adapter) {
        const eventHandlers = this.handlers.get(event) ?? [];
        for (const handler of eventHandlers) {
            await handler(activity, adapter);
        }
    }
    $onMessage(handler) { this.register('message', handler); }
    $onMention(handler) { this.register('mention', handler); }
    $onThreadReplyAdded(handler) { this.register('threadReplyAdded', handler); }
    $onDMReceived(handler) { this.register('dmReceived', handler); }
    $onReactionAdded(handler) { this.register('reactionAdded', handler); }
    $onReactionRemoved(handler) { this.register('reactionRemoved', handler); }
    $onCardAction(handler) { this.register('cardAction', handler); }
    $onInvoke(handler) { this.register('invoke', handler); }
    $onMessageAction(handler) { this.register('messageAction', handler); }
    $onMemberAdded(handler) { this.register('memberAdded', handler); }
    $onMemberRemoved(handler) { this.register('memberRemoved', handler); }
    $onTeamRenamed(handler) { this.register('teamRenamed', handler); }
    $onChannelCreated(handler) { this.register('channelCreated', handler); }
    $onChannelRenamed(handler) { this.register('channelRenamed', handler); }
    $onChannelDeleted(handler) { this.register('channelDeleted', handler); }
    $onAppInstalled(handler) { this.register('appInstalled', handler); }
    $onAppUninstalled(handler) { this.register('appUninstalled', handler); }
    $onBotActivity(handler) { this.register('botActivity', handler); }
    async processActivity(activity, adapter) {
        if (!adapter)
            return;
    ...
    async routeMessageActivity(activity, adapter) {
        const isMention = (activity.entities ?? []).some((e) => e.type === 'mention' && e.mentioned?.id === activity.recipient.id);
    ...
    async routeConversationUpdate(activity, adapter) {
...
export class TeamsAdapterImpl {
    ...
    async postMessage(threadId, message) {
        ...
        const response = await this.callWithRetry(() => fetch(url, {
            method: 'POST',
    ...
    async editMessage(threadId, messageId, message) {
        ...
        const token = await this.ensureAccessToken();
        await this.callWithRetry(() => fetch(url, {
            method: 'PUT',
            headers: this.botConnectorHeaders(token),
            body: JSON.stringify(activity),
        }));
    ...
    async fetchMessages(threadId) {
        ...
        if (!context.teamId || !context.channelId) {
            throw new TeamsAdapterError('fetchMessages requires teamId and channelId in the thread context', TeamsAdapterErrorCode.VALIDATION_ERROR, 400);
        }
        ...
        return raw.map((item) => this.graphMessageToMessage(item));
    }
    async fetchMessage(threadId, messageId) {
        const context = decodeThreadId(threadId);
        if (!context.teamId || !context.channelId) {
            throw new TeamsAdapterError('fetchMessage requires teamId and channelId in the thread context', TeamsAdapterErrorCode.VALIDATION_ERROR, 400);
        }
        const graphClient = await this.createGraphClient();
        const raw = await graphClient.getChannelMessage(context.teamId, context.channelId, messageId);
        return this.graphMessageToMessage(raw);
    }
    ...
    async postChannelMessage(channelId, message) {
        ...
        const result = (await graphClient.sendChatMessage(`${teamId}/channels/${chanId}`, body));
    ...
    async addReaction(threadId, messageId, emoji) {
        const context = decodeThreadId(threadId);
        if (!context.teamId || !context.channelId) {
            throw new TeamsAdapterError('addReaction requires teamId and channelId in the thread context', TeamsAdapterErrorCode.VALIDATION_ERROR, 400);
        }
        const graphClient = await this.createGraphClient();
    ...
    async removeReaction(threadId, messageId, emoji) {
        const context = decodeThreadId(threadId);
        if (!context.teamId || !context.channelId) {
            throw new TeamsAdapterError('removeReaction requires teamId and channelId in the thread context', TeamsAdapterErrorCode.VALIDATION_ERROR, 400);
        }
        const graphClient = await this.createGraphClient();
    ...
    async fetchChannelInfo(channelId) {
        const [teamId, chanId] = parseCompositeChannelId(channelId);
        const graphClient = await this.createGraphClient();
        const raw = (await graphClient.getChannel(teamId, chanId));
        return {
            id: raw.id ?? chanId,
    ...
    async openDM(userId) {
        const graphClient = await this.createGraphClient();
        const chat = (await graphClient.createOneOnOneChat(userId, this.config.appId));
        if (!chat.id) {
    ...
    async openModal(_triggerId, modal) {
    ...
    buildCredentials() {
        ...
        throw new TeamsAdapterError('No credentials configured. Provide appPassword or appCertificate.', TeamsAdapterErrorCode.UNAUTHORIZED, 401);
    }
    // ---------------------------------------------------------------------------
    // Private: HTTP helpers
    // ---------------------------------------------------------------------------
    botConnectorHeaders(token) {
        return {
     ...
     */
    async callWithRetry(fn, retries = this.config.maxRetries ?? 3, delayMs = this.config.retryDelayMs ?? 500) {
        let lastError;
 ...
 */
export function createTeamsAdapter(config) {
...
function validateConfig(config) {
    if (!config.appId) {
        throw new TeamsAdapterError('TeamsAdapterConfig.appId is required', TeamsAdapterErrorCode.VALIDATION_ERROR, 400);
    }
}
/**
 ...
 */
function parseCompositeChannelId(id) {
...
function buildGraphMessageBody(message) {
    return {
        body: {
