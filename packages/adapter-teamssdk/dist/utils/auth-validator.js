...
const BOT_FRAMEWORK_OPENID_CONFIG = 'https://login.botframework.com/v1/.well-known/openidconfiguration';
/**
 ...
 */
export async function validateToken(token, appId) {
    try {
        ...
        const isValidIssuer = typeof iss === 'string' &&
 ...
 */
async function verifyCryptoSignature(token, jwk) {
    try {
        const parts = token.split('.');
        ...
        const cryptoKey = await globalThis.crypto.subtle.importKey('jwk', {
            kty: 'RSA',
            n: jwk.n,
            e: jwk.e,
            alg: 'RS256',
        ...
        return await globalThis.crypto.subtle.verify('RSASSA-PKCS1-v1_5', cryptoKey, signatureBytes, signingInputBytes);
    }
    catch {
        return false;
    }
...
export async function getAccessToken(credentials) {
    ...
    switch (credentials.type) {
        ...
        case 'certificate': {
            ...
            const assertion = await buildCertificateAssertion(credentials.appId, credentials.thumbprint, credentials.privateKey, tokenUrl);
            body = new URLSearchParams({
                grant_type: 'client_credentials',
                client_id: credentials.appId,
        ...
        default:
            throw new Error(`Unsupported credential type: ${String(credentials.type)}`);
    }
 ...
 */
async function buildCertificateAssertion(appId, thumbprint, privateKeyPem, audience) {
    const now = Math.floor(Date.now() / 1000);
    const header = {
        alg: 'RS256',
        typ: 'JWT',
    ...
    const cryptoKey = await globalThis.crypto.subtle.importKey('pkcs8', keyBuffer, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['sign']);
    const signatureBuffer = await globalThis.crypto.subtle.sign('RSASSA-PKCS1-v1_5', cryptoKey, new TextEncoder().encode(signingInput));
