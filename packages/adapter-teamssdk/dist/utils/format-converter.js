 ...
 */
export function activityToMessage(activity) {
    const message = {
        id: activity.id,
 ...
 */
export function messageToActivity(message) {
    const activity = {
        type: 'message',
    };
 ...
 */
export function adaptiveCardToAttachment(card) {
    return {
 ...
 */
function markdownToTeamsHtml(md) {
    let html = md;
    // Fenced code blocks (``` ... ```)
    html = html.replace(/```(\w*)\n?([\s\S]*?)```/g, (_m, __lang, code) => {
    ...
    return blocks
        ...
        switch (block.type) {
            ...
            case 'context': {
                ...
                const texts = elements.map((el) => typeof el === 'object' && el !== null && 'text' in el
                    ? String(el['text'])
                    : '');
 ...
 */
function teamsAttachmentToAttachment(ta) {
    if (ta.contentType === 'application/vnd.microsoft.card.adaptive' ||
 ...
 */
function attachmentToTeamsAttachment(att) {
    switch (att.type) {
        case 'card':
            return {
