 ...
 */
export function encodeThreadId(context: TeamsContext): string {
 ...
 */
export function decodeThreadId(threadId: string): TeamsContext {
  try {
    ...
    return parsed as TeamsContext;
  } catch (err) {
    throw new Error(
      `Failed to decode thread ID: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
